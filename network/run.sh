...
ORDERER_CA=/opt/src/github.com/hyperledger/ehr-fabric/network/organizations/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem

DOCKER_SOCK="${DOCKER_SOCK}" ${CONTAINER_CLI_COMPOSE} ${COMPOSE_FILES} up -d 2>&1
docker-compose -f compose/docker-compose.yaml  up -d 2>&1
...
cryptogen generate --config=./organizations.yaml --output="organizations"
if [ "$?" -ne 0 ]; then
